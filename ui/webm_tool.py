...
class WebMToolView(QWidget):
    ...
    def init_ui(self):
        ...
        title = QLabel("Video → WebM Mobile Stream")
        title.setProperty("class", "ToolTitle")
        desc = QLabel("Compress heavy videos into lightweight, instant-loading WebM files for fast web layouts.")
        ...
        self.dest_desc = QLabel("Compressed videos will be saved in 'optimized_webm_output' folders alongside each source video.")
        ...
        # Output Width Scaling
        width_layout = QHBoxLayout()
        width_lbl = QLabel("Output Width Scaling:")
        width_lbl.setProperty("class", "FormLabel")
        width_lbl.setFixedWidth(150)
        self.width_input = QLineEdit()
        self.width_input.setText("1280:-2")
        self.width_input.setPlaceholderText("e.g., 1280:-2 or 1920:-2")
        width_layout.addWidget(width_lbl)
        width_layout.addWidget(self.width_input)
        self.adv_settings.add_layout(width_layout)
        
        # Frame Rate
        fps_layout = QHBoxLayout()
        fps_lbl = QLabel("Frame Rate (FPS):")
        fps_lbl.setProperty("class", "FormLabel")
        fps_lbl.setFixedWidth(150)
        self.fps_input = QLineEdit()
        self.fps_input.setText("20")
        self.fps_input.setPlaceholderText("e.g., 20 or 24 or 30")
        fps_layout.addWidget(fps_lbl)
        fps_layout.addWidget(self.fps_input)
        self.adv_settings.add_layout(fps_layout)
        
        # CRF Slider
        crf_layout = QHBoxLayout()
        crf_lbl = QLabel("CRF Value (Quality):")
        crf_lbl.setProperty("class", "FormLabel")
        crf_lbl.setFixedWidth(150)
        self.crf_slider = QSlider(Qt.Horizontal)
        self.crf_slider.setRange(4, 63)
        self.crf_slider.setValue(36)
        self.crf_val_lbl = QLabel("36")
        self.crf_val_lbl.setProperty("class", "FormValue")
        self.crf_val_lbl.setFixedWidth(30)
        self.crf_slider.valueChanged.connect(lambda v: self.crf_val_lbl.setText(str(v)))
        crf_layout.addWidget(crf_lbl)
        crf_layout.addWidget(self.crf_slider, 1)
        crf_layout.addWidget(self.crf_val_lbl)
        self.adv_settings.add_layout(crf_layout)
        
        # Target Bitrate
        bitrate_layout = QHBoxLayout()
        bitrate_lbl = QLabel("Target Bitrate:")
        bitrate_lbl.setProperty("class", "FormLabel")
        bitrate_lbl.setFixedWidth(150)
        self.bitrate_input = QLineEdit()
        self.bitrate_input.setText("800k")
        self.bitrate_input.setPlaceholderText("e.g., 800k or 1.5M")
        bitrate_layout.addWidget(bitrate_lbl)
        bitrate_layout.addWidget(self.bitrate_input)
        self.adv_settings.add_layout(bitrate_layout)
        
        # GOP size
        gop_layout = QHBoxLayout()
        gop_lbl = QLabel("GOP / Keyframe Int:")
        gop_lbl.setProperty("class", "FormLabel")
        gop_lbl.setFixedWidth(150)
        self.gop_input = QLineEdit()
        self.gop_input.setText("1")
        self.gop_input.setPlaceholderText("e.g., 1 (scroll sync) or 250")
        gop_layout.addWidget(gop_lbl)
        gop_layout.addWidget(self.gop_input)
        self.adv_settings.add_layout(gop_layout)
        
        # Audio inclusion Toggle
        self.audio_checkbox = QCheckBox("Include Audio Stream")
        self.audio_checkbox.setChecked(False)
        self.adv_settings.add_widget(self.audio_checkbox)
        ...
        self.log_terminal.setPlaceholderText("Live subprocess encoding activity will output here...")
    ...
    def reset_defaults(self):
        ...
        self.gop_input.setText("1")
        self.audio_checkbox.setChecked(False)
        self.log_terminal.append("Settings reset to original hardcoded compression defaults.")
    ...
    def run_workflow(self):
        ...
        self.worker = WebMWorker(
            source_paths=paths,
            scale=self.width_input.text(),
            fps=self.fps_input.text(),
            crf=self.crf_slider.value(),
            bitrate=self.bitrate_input.text(),
            g=self.gop_input.text(),
    ...
    def parse_and_report_stats(self):
        ...
        import re
        import os
        successes = re.findall(r"Success! Extreme optimized mobile asset built at:\n(.*)", text)
        if successes:
            count = len(successes)
            mb_saved = 0.0
            for dest_path in successes:
                dest_path = dest_path.strip()
                if os.path.exists(dest_path):
                    try:
                        out_size = os.path.getsize(dest_path) / (1024 * 1024)
                        mb_saved += out_size * 3.0
                    except Exception:
                        pass
